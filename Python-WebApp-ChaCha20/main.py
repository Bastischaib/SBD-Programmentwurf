...
def chacha20_encrypt(message):
    ...
    encoded_ciphertext = urlsafe_b64encode(ciphertext).decode()
...
def message(data):
    ...
    encoded_ciphertext = chacha20_encrypt(content["message"])

    send({"name": content["name"], "message": content["message"]}, to=room)
    rooms[room]["messages"].append({"name": content["name"], "message": content["message"]})
    print(f"{session.get('username')} said: {encoded_ciphertext}")

    new_message = Message(content=encoded_ciphertext, user_id=User.query.filter_by(username=content["name"]).first().id)
